...
- **`AuthController.refresh` must fall back to the `PersistentTokenAuthFilter`-set principal**: fixed alongside the gotcha above. The endpoint originally required a live `refresh_token` cookie (7 days) and returned 401 the instant it was missing/expired — even when the request arrived with a still-valid `persistent_token` (90 days) that `PersistentTokenAuthFilter` had *already* used to re-authenticate the request and mint fresh cookies on the response, one filter earlier in the chain. `refresh` now accepts `@AuthenticationPrincipal AppUser` and returns success from that principal when no `refresh_token` cookie is present, instead of ignoring the filter's work.
